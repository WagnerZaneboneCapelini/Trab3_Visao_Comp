...
ENV VIRTUAL_ENV /opt/Trab3_Visao_Comp-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/Trab3_Visao_Comp/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/Trab3_Visao_Comp/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/trab3_visao_comp/ && touch src/trab3_visao_comp/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/Trab3_Visao_Comp/
